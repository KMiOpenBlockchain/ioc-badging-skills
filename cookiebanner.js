function runAfterHTMLIncluded() {
    ...
    if (!localStorage.getItem('openBlockchainIoCBadgingSkillscookieConsent')) {
        cookieBanner.style.display = 'block';
    }

    // Function to check if the user has given consent
    function hasConsent() {
        const consent = localStorage.getItem('openBlockchainIoCBadgingSkillscookieConsent');
        return consent !== null && consent === 'true';
    }

    noCookies.addEventListener('click', function () {
        localStorage.setItem('openBlockchainIoCBadgingSkillscookieConsent', 'true');
